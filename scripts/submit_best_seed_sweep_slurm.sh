...
#SBATCH -t 120:00:00
#SBATCH -o vcat_seed_sweep_%j.out
#SBATCH -e vcat_seed_sweep_%j.err
...
BASE_OUTPUT=${BASE_OUTPUT:-"$SLURM_SUBMIT_DIR/outputs_best_${SPLIT_MODE}_seeds_${SLURM_JOB_ID}"}
...
for ((offset = 0; offset < NUM_SEEDS; offset++)); do
  seed=$((START_SEED + offset))
  split_file="$SPLIT_DIR/${SPLIT_PREFIX}_${SPLIT_MODE}_seed${seed}.csv.gz"
  output_dir="$BASE_OUTPUT/seed${seed}"

  echo "============================================================"
  echo "[RUN] seed=$seed split_file=$split_file"
  echo "      balance=$BALANCE_STRATEGY balance_splits=$BALANCE_SPLITS"
  echo "      tcs_standardization=$TCS_STANDARDIZATION"
  echo "      drug_feature=$DRUG_FEATURE smiles_csv=$SMILES_CSV"
  echo "      ablation=$ABLATION"
  echo "      batch=$BATCH lr=$LR wd=$WD dropout=$DROPOUT label_smoothing=$LABEL_SMOOTHING"
  echo "      mc_dropout_passes=$MC_DROPOUT_PASSES output=$output_dir"
...
python scripts/summarize_seed_results.py --base_output "$BASE_OUTPUT"
echo "[DONE] seed sweep complete: $BASE_OUTPUT"
