...
#SBATCH -t 120:00:00
#SBATCH -o vcat_vpm_sweep_%j.out
#SBATCH -e vcat_vpm_sweep_%j.err
...
BASE_OUTPUT=${BASE_OUTPUT:-"$SLURM_SUBMIT_DIR/outputs_vpm_strategy_${SPLIT}_seed${SEED}_${SLURM_JOB_ID}"}
...
run_strategy() {
  local strategy="$1"
  local output_dir="$BASE_OUTPUT/$strategy"
  echo "============================================================"
  echo "[RUN] strategy=$strategy seed=$SEED split=$SPLIT"
  echo "      vpm_lr_multiplier=$VPM_LR_MULTIPLIER unfreeze_epoch=$VPM_UNFREEZE_EPOCH"
  echo "      output=$output_dir"
  echo "============================================================"
  python scripts/train.py \
    "${COMMON_ARGS[@]}" \
    --vpm_finetune_strategy "$strategy" \
    --output_dir "$output_dir"
}

run_strategy frozen
run_strategy unfreeze_all
run_strategy unfreeze_after_warmup

python scripts/summarize_strategy_results.py --base_output "$BASE_OUTPUT"
echo "[DONE] VPM strategy sweep complete: $BASE_OUTPUT"
